...
const fieldHandlers = {
  ...
  "機關資料:機關地址": (addressString, ocdsRelease) => {
    // get county name
    const countyLastCharIndex = addressString.indexOf("市") || addressString.indexOf("縣");
    if (countyLastCharIndex > 0) {
      ...
      console.error(`county name is not found in address string ${addressString}`);
    }
    ...
    if (districtLastCharIndex > 0) {
      ...
      console.error(`district name is not found in address string ${addressString}`);
    }
